...
while (input !== commands[3]) {
  input = prompt("「" + commands + "」のうちいずれかひとつを入力してください");
  if (input === commands[0]) {
    showTodos();
...
// ここにdeleteTodo関数を作る
function deleteTodo() {
  const dIndex = prompt("削除するタスクの番号を指定してください");
  const parsedNumber = parseInt(dIndex, 10);
  if (parsedNumber >= 0 && parsedNumber < todos.length) {
